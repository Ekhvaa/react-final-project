...
const HomePage = () => {
  const apiUrl = `https://tourist-platform-api-f9g6c9azezbvehf0.italynorth-01.azurewebsites.net/api/tours?PageSize=${TRENDING_TOURS_LIMIT}`;
  const { data: tours, isLoading, error } = useFetch(apiUrl, (resData) => resData?.items || []);
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="flex justify-center mt-14">
              <Link
                to="/tours"
                className='cursor-pointer rounded-xl bg-[#8dfc9c] px-8 py-3 hover:bg-[#39bf00] font-semibold transition-all duration-300'
              >
                See More Tours
              </Link>
            </div>
